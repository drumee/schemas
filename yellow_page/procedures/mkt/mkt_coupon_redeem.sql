...
proc: BEGIN
  ...
  IF _norm = '' OR _em = '' OR _pl = '' OR IFNULL(_org_id, '') = '' THEN
  ...
  SELECT quota INTO _plan_quota FROM yp.plan
    WHERE plan_code = _pl AND entity_type = 'org' AND active = 1
    LIMIT 1;
  ...
  VALUES
    (_domain_id, _org_id, _pl, _plan_quota, 'mkt-coupon', _period_end, _now, _now)
  ...
  INSERT INTO mkt_coupon_redemption
    (coupon_id, code, partner, email, uid, plan, period, entity_type,
     ...
     ctime, mtime)
  VALUES
    (_cid, _norm, IFNULL(_partner, ''), _em, _uid, _pl, 'trial', 'org',
     'confirmed', _now, _now, _period_end, _org_id, _domain_id, _now, _now);
