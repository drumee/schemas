...
--
-- The body lives in aha_moment_window; this delegates with no window, so there
-- is one definition and no copy to drift. NULL, not '{}': JSON_VALUE(NULL, ...)
-- is NULL, so both bounds read as absent.
-- =========================================================
DROP PROCEDURE IF EXISTS `aha_moment`$
CREATE PROCEDURE `aha_moment`()
BEGIN
  CALL aha_moment_window(NULL);
END $
