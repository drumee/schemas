...
--
-- The body lives in core_function_window; this delegates with no window, so there
-- is one definition and no copy to drift. NULL, not '{}': JSON_VALUE(NULL, ...)
-- is NULL, so both bounds read as absent.
-- =========================================================
DROP PROCEDURE IF EXISTS `core_function`$
CREATE PROCEDURE `core_function`()
BEGIN
  CALL core_function_window(NULL);
END $
