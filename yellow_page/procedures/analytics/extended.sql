...
--
-- The body lives in extended_window; this delegates with no window, so there
-- is one definition and no copy to drift. NULL, not '{}': JSON_VALUE(NULL, ...)
-- is NULL, so both bounds read as absent.
-- =========================================================
DROP PROCEDURE IF EXISTS `extended`$
CREATE PROCEDURE `extended`()
BEGIN
  CALL extended_window(NULL);
END $
