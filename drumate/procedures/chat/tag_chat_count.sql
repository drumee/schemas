...
BEGIN
  ...
  SELECT
    ...
    WHERE 
      CASE 
        WHEN _tag_id IS NOT NULL AND _tag_id <> ''  THEN 
          c.id IN (SELECT id FROM map_tag mt WHERE mt.tag_id = _tag_id) 
        ELSE c.id =c.id 
      END 
    AND c.uid IS NOT NULL;

    INSERT INTO 
    _show_node(entity_id,hub_id,flag,db_name)
    SELECT m.id ,m.id , 'share', db_name    
    FROM media m
      INNER JOIN yp.hub h on  h.id = m.id 
      INNER  JOIN yp.entity he ON m.id = he.id
    WHERE category = 'hub' AND 
      CASE 
        WHEN _tag_id IS NOT NULL AND _tag_id <> '' THEN 
    ...
    WHILE _nid IS NOT NULL DO 
      ...
      UPDATE 
        _show_node SET room_count =  _read_cnt
        WHERE entity_id = _nid ;

      UPDATE _show_node SET is_checked = 1 WHERE entity_id = _nid ;
      SELECT NULL , NULL  INTO _read_cnt,_nid;
      SELECT entity_id, db_name FROM _show_node 
        WHERE is_checked =0 LIMIT 1 INTO _nid, _db_name; 
     END WHILE;
