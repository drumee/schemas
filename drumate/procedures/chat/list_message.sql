...
BEGIN
  ...
  CALL acknowledge_message(JSON_MERGE(
  ...
  SELECT _page as `page`,
    ...
    CASE 
      WHEN LTRIM(RTRIM(ch.attachment))='' OR  ch.attachment IS NULL THEN 0 
      ELSE 1 
    END is_attachment, 
    ch.status,
    ch.ctime,
    ch.metadata,
    CASE 
      WHEN _ref_sys_id < ch.sys_id THEN 1 
      ELSE 0 
    END is_notify,  
    CASE 
      WHEN my_read.ref_sys_id >= ch.sys_id THEN 1
      ELSE 0 
    END is_readed,
    CASE 
      WHEN his_read.ref_sys_id >= ch.sys_id THEN 1 
      ELSE 0 
    END is_seen,
