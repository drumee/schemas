...
BEGIN
  ...
  SELECT _entity_id, _uid, _ref_sys_id, UNIX_TIMESTAMP() 
    ON DUPLICATE KEY 
    UPDATE ref_sys_id= _ref_sys_id, ctime =UNIX_TIMESTAMP();
  ...
  INSERT INTO read_channel(entity_id, `uid`, ref_sys_id, ctime)
    SELECT _entity_id,_entity_id,0,UNIX_TIMESTAMP() 
    WHERE _old_ref_sys_id IS NULL;

  SELECT db_name FROM yp.entity 
    WHERE id=_entity_id AND status <> 'deleted' INTO _entity_db;
  IF _entity_db  IS  NOT NULL THEN 
    SET @st = CONCAT(
      'CALL ', _entity_db ,'.post_acknowledge_message(?)'
    );
    PREPARE stamt FROM @st;
    EXECUTE stamt USING JSON_MERGE(
