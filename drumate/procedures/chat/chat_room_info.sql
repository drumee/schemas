...
BEGIN
  ...
  SELECT
    ...
    FROM
      ...
      LEFT JOIN yp.contact_block hiscb ON (hiscb.owner_id =  c.entity 
        OR hiscb.owner_id = c.uid) 
        AND( hiscb.uid = _uid 
        OR hiscb.entity = _uid OR hiscb.entity = _mail 
      ) 
  ...
  INSERT INTO 
    _show_node(entity_id,hub_id,display,flag, `message`, ctime, `status`,is_archived)
